...
workflow BIN_ANNOTATION {
    ...
    if (params.bin_annotation_tool == 'bakta') {
        ...
        if (params.annotation_bakta_db) {
            ...
            ch_versions = ch_versions.mix(BAKTA_BAKTADBDOWNLOAD.out.versions_bakta)
        ...
        ch_versions = ch_versions.mix(BAKTA_BINS.out.versions_bakta)
        ch_multiqc_files = ch_multiqc_files.mix(
            BAKTA_BINS.out.txt.map { output -> output[1] }.flatten()
        )
