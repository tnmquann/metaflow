...
workflow BINNING_BAMABUND {
    ...
    ch_assembly_reads_paired = ch_assemblies_by_sample
        ...
        .map { _sample_id, assembly_meta, assembly, reads_meta, reads ->
    ...
    if (params.binprepare_mappertool == 'bwamem2') {
        ...
        BWAMEM2_ASSEMBLY_INDEX(
            ch_assembly_reads_paired.map { meta, assembly, _reads -> [meta, assembly] }
        )
        versions_ch = versions_ch.mix(BWAMEM2_ASSEMBLY_INDEX.out.versions_bwamem2)
        ...
        ch_bwamem2_input = ch_assembly_reads_paired
            ...
            .map { _key, reads_meta, reads, assembly, _index_meta, index ->
                [reads_meta, reads, index, assembly]
            }

        BWAMEM2_ASSEMBLY_MEM(
            ch_bwamem2_input.map { meta, reads, _index, _fasta -> [meta, reads] },
            ch_bwamem2_input.map { meta, _reads, index, _fasta -> [meta, index] },
            ch_bwamem2_input.map { meta, _reads, _index, fasta -> [meta, fasta] },
            false
        )
        versions_ch = versions_ch
            .mix(BWAMEM2_ASSEMBLY_MEM.out.versions_bwamem2)
            .mix(BWAMEM2_ASSEMBLY_MEM.out.versions_samtools)
        ...
        BOWTIE2_ASSEMBLY_BUILD(
            ch_assembly_reads_paired
                .map { meta, assembly, _reads -> [meta, assembly] }
                .unique { entry -> entry[0].id + "_" + entry[0].assembler }  // Only build index once per assembly
        )
        versions_ch = versions_ch.mix(BOWTIE2_ASSEMBLY_BUILD.out.versions_bowtie2)
        ...
        BOWTIE2_ASSEMBLY_ALIGN(
            ch_bowtie2_input.map { meta, reads, _index, _fasta -> [meta, reads] },
            ch_bowtie2_input.map { meta, _reads, index, _fasta -> [meta, index] },
            ch_bowtie2_input.map { meta, _reads, _index, fasta -> [meta, fasta] },
            false,
            false
        )
        versions_ch = versions_ch
            .mix(BOWTIE2_ASSEMBLY_ALIGN.out.versions_bowtie2)
            .mix(BOWTIE2_ASSEMBLY_ALIGN.out.versions_samtools)
            .mix(BOWTIE2_ASSEMBLY_ALIGN.out.versions_pigz)
