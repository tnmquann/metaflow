...
workflow BINNING {
    ...
    ch_for_binning = assemblies
        ...
        .map { _key, meta, assembly, bams, bais ->
    ...
    if (!params.skip_metabat2) {
        ch_summarizedepth_input = ch_for_binning.map { meta, _assembly, bams, bais ->
            [meta, bams, bais]
        }

        METABAT2_JGISUMMARIZEBAMCONTIGDEPTHS(ch_summarizedepth_input)
        versions_ch = versions_ch.mix(METABAT2_JGISUMMARIZEBAMCONTIGDEPTHS.out.versions_metabat2)
        ...
        split_fasta_script = file("${projectDir}/bin/py_scripts/assembly_based/split_fasta.py", checkIfExists: true)
        SPLIT_FASTA(ch_metabat2_unbinned, split_fasta_script)
    ...
    if (!params.skip_concoct) {
        ...
        ch_assemblies_for_gunzip = ch_for_binning.map { meta, assembly, _bams, _bais ->
            def meta_new = meta.clone()
        ...
        ch_concoct_input = ch_for_binning
            ...
            .map { _key, meta, _assembly_gz, bams, bais, fasta_uncompressed ->
    ...
    if (!params.skip_semibin) {
        ...
        versions_ch = versions_ch.mix(SEMIBIN_SINGLEEASYBIN.out.versions_semibin)
