process PROCESS_READBASED_RESULTS {
    ...
    tuple val(_meta_yacht), path(yacht_xlsx)
    path process_sourmash_results_script
    path process_yacht_results_script
    path concat_yacht_xlsx_script
    path merged_yacht_sourmash_script
    ...
    python ${process_sourmash_results_script} \\
        -i temp_text_files/sourmash_gather_withrocksdb.csv \\
        -o temp_text_files/sourmash_mergedresults.csv \\
        $py_args_script1

    python ${process_yacht_results_script} \\
        -d temp_yacht_results \\
        $py_args_script2

    python ${concat_yacht_xlsx_script} \\
        -d temp_yacht_results \\
        $py_args_script3

    python ${merged_yacht_sourmash_script} \\
        -d1 final_results \\
        -d2 temp_text_files \\
